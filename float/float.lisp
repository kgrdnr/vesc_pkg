...
;(def led-num 44)
(define num-led-hud (ext-float-dbg 31))
(define led-num (+ (ext-float-dbg 20) (ext-float-dbg 21) num-led-hud))
            ...
            (if (= led-on 1)    
                (if (> switch-state 0)
                    (progn
                        ...
                        (if (< i num-led-hud)
                            (if (< i (* num-led-hud (abs (get-duty))))
                                (ext-ws2812-set-color i (ix colors 2))
                                (ext-ws2812-set-color i 0)
                            )
                            (if (= forward-movement 1)
                                (if (< i (+ num-led-front num-led-hud))
