...
// Settings
#define WS2812_CLK_HZ		800000 //800000
//#define WS2812_CLK_HZ		1000
#define TIM_PERIOD			(((168000000 / 2 / WS2812_CLK_HZ) - 1))
#define WS2812_ZERO			(TIM_PERIOD * 0.3)	//0.4
#define WS2812_ONE			(TIM_PERIOD * 0.7)	//0.7
...
static uint32_t rgb_to_local(ws_cfg *cfg, uint32_t color) {
	...
	if (cfg->bits == 32) {
		if(cfg->RGswap) return (r << 24) | (g << 16) | (b << 8) | w;
		else return (g << 24) | (r << 16) | (b << 8) | w;
	} else {
		if(cfg->RGswap) return (r << 16) | (g << 8) | b;
		else return (g << 16) | (r << 8) | b;
	}
}
...
static lbm_value ext_init(lbm_value *args, lbm_uint argn) {
	if (argn != 5 || !VESC_IF->lbm_is_number(args[0]) || !VESC_IF->lbm_is_number(args[1]) ||
	...
	if (cfg) {
		...
		cfg->RGswap = VESC_IF->lbm_dec_as_i32(args[4]);
